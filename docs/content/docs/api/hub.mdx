...
| `set_user_best(coral_dir, commit_hash)` | Mark one attempt as the user-selected best and clear prior marks |
...
Module: `coral.hub.steering`

Steering actions are JSON files in `.coral/public/steering/`. They are written
with tmp-file + rename and read by `coral resume`.

### Functions

| Function | Description |
|----------|-------------|
| `enqueue(coral_dir, action)` | Write a pending steering action |
| `read_pending(coral_dir)` | Read unapplied actions sorted by creation time |
| `mark_applied(coral_dir, id)` | Mark one action applied |
...
| `mark_best` | User-best selection; the web API applies this immediately via attempt metadata |
